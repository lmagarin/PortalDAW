...
<html lang="es">
    <head>
        <title>Generar SQL desde Fichero</title>
        <meta charset="utf-8"/>
    ...
    <body>   
        <header>
           <h1>Generar SQL desde Fichero</h1>
        </header>
        <section>
            <?php
                if(!isset($_POST['enviar'])){
                    echo "<form enctype='multipart/form-data' action='SQLdesdeFichero.php' method='post'>
                            <fieldset>
                                <legend> Subida de archivos</legend>
                                    Archivo a subir:<input type='file' name='archivo' value='Subir archivo'><br><br>
                                    Nombre del archivo (no incluir extensión):<input type='text' name='sql' value=''><br><br>
                                    Identificador final para usuarios:
                                        <select name='sufijo'>
                                            <option value='1_daw'>1_daw</option>
                                            <option value='2_daw'>2_daw</option>
                                            <option value='1_asir'>1_asir</option>
                                            <option value='2_asir'>2_asir</option>
                                        </select><br><br>
                    ...
                    if(isset($_POST['sufijo'])){
                        $idUsuarios = "";
                        $idUsuarios = $_POST['sufijo'];
                    }
                    $ruta_subida = "/var/www/rafa/ejerciciosServidor/Ficheros/";
                    ...
                    if($extension == "txt") {
                        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $ruta_subida)) {
                            ...
                            foreach ($arrayEstudiantes as $valor) { //recorro el array
                                ...
                                $codigoCompleto = $codigoApellido1 . $codigoApellido2 . $codigoNombre . $idUsuarios; //creo el codigo completo del alumno
                        ...
                        echo "Error, solo se pueden subir archivos de extensión .txt";
                    }
                }
            ?>
        </section>
